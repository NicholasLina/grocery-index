 ...
 */
const StatCanSchema = new mongoose_1.default.Schema({
 ...
 */
const PriceChangeSchema = new mongoose_1.default.Schema({
    /** Product name */
    product: String,
    /** Geographic location */
    geo: String,
 ...
 */
async function calculateAndStorePriceChanges(geo) {
    ...
    try {
        ...
        for (const product of products) {
            try {
                ...
                await PriceChange.findOneAndUpdate({ product, geo }, {
                    product,
                    geo,
                    currentPrice,
                    previousPrice,
 ...
 * { "geo": "Canada" }
 */
router.post('/calculate-changes', async (req, res) => {
    const { geo } = req.body;
    if (!geo) {
        return res.status(400).json({ error: 'Geographic location (geo) is required' });
    }
    ...
    try {
        ...
        for (const geo of geoValues) {
            try {
                ...
                results.push({ geo, processedCount: 0, success: false, error: err.message });
            }
        }
